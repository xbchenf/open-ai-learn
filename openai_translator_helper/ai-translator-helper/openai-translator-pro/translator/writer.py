...
from document import Book, ContentType
...
class Writer:
    ...
    def _save_translated_book_pdf(self, book: Book, output_file_path: str = None):
                ...
                if content.status:
                    if content.content_type == ContentType.TEXT:
                        for paragraph in content.paragraphs:
                            #LOG.info(f"paragraph:{paragraph.text}")
                            #LOG.info(f"translation:{paragraph.translation}")
        ...
        # Save the translated document as a new PDF file
        doc.build(story)
